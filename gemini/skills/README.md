...
| semantic search vs file read | **语义搜索优先**，ReadFile 仅用于读取定位到的文件 |
...
```
1️⃣ Semantic Search / Codebase Search  ← MUST TRY FIRST
    ↓ (only if failed)
2️⃣ rg / grep (pattern search)
    ↓ (for specific content)
3️⃣ Read File (read located files)
```

**禁止行为：**
- ❌ 跳过语义搜索直接用 ReadFile 探索代码
- ❌ 在语义搜索可用时使用 grep 进行语义搜索
- ❌ 不尝试高级搜索就使用降级方案
...
```
Semantic search FIRST → Fallback to rg/grep → Then ReadFile
MUST try semantic search before any code exploration
...
|-------|---------|--------------|
| prometheus-core | 5.1.0 | 2025-01-08 |
| prometheus-aether-ui | 1.0.0 | 2025-01-08 |
