...
2. **上下文收集 `<context_gathering>`（ACE 优先）**
   
   **⚡ 必须优先使用 `ace-tool` 获取上下文：**
   ```
   调用 ace-tool:
     - query: "分析 <issue 描述> 涉及的文件、依赖关系、影响范围"
   ...
   **🔄 ACE 降级策略（仅当 ace-tool 不可用时）：**
   ...
   - 降级标记：在 `notes` 中记录 `context_mode:fallback <原因>`
   
   **早停条件：** 能够命名"要修改哪些具体文件/函数"，即可进入实现；仅在验证失败或出现新未知时再回到收集。
3. **执行（实现 + 文档同步）**
...
3. **ACE 上下文加载（必须执行）**
   ...
   **⚡ 优先调用 `ace-tool`：**
   ```
   调用 ace-tool:
     - query: "分析 <title> 的实现：涉及文件、依赖、调用链、影响范围"
     - project_root_path: <项目根目录绝对路径>
   ```
   ...
   **🔄 ACE 不可用时的降级方案：**
   | ACE 能力 | 降级方案 |
   |----------|----------|
   | 依赖分析 | `rg "import.*<symbol>"` + 手动追踪 |
   | 符号搜索 | `rg "<symbol>"` |
   | 影响范围 | 从 `refs` 入手 + 手动分析调用链 |
...
6. **深度上下文收集（基于 ACE 结果）**
   - 根据 ACE 返回的文件列表，优先从 `refs` 指向文件开始读；
   - 若 ACE 不可用：使用 `rg` 精确定位关键符号与调用链；
   - 早停：能明确"要改哪些具体文件/函数"即可进入实现。
8. **实现（重点：按验收口径驱动） + 文档同步**
   1. **实现前确认**
...
10. **自我验收（严格按 acceptance_criteria）**
    - 给出"通过/未通过"的证据；
...
11. **完成状态并写回 CSV（提交前，确保同 commit 演进）**
